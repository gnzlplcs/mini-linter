...
console.log(`There are ${betterWords.length} better words.`)
...
const printInfo = (origArr, arrToBeFiltered) => {
  let printResult = '';
  arrToBeFiltered.forEach(word => {
    let times = countTimes(origArr, word);
    printResult += `The array contains "${word}" ${times} ${times > 1 ? 'times' : 'time'}.
  `
  })
  return printResult;
}

console.log(printInfo(storyWords, overusedWords))
...
console.log(`There are ${countSentences(storyWords)} sentences.`);
...
console.log(`
  The story contains ${storyWords.length} words.
  It contains ${countSentences(storyWords)} sentences.
  ${printInfo(storyWords, overusedWords)}
`)
